...
sol.simplealt <- nloptr(x0=c(1,1), eval_f=UtilSimple, lb=c(0,0), ub=c(50,20),
   util.type="cobb", alpha=alpha, p.x=p.x, p.y=p.y, M=M, gamma = gamma,
   opts= list("algorithm"="NLOPT_GN_ISRES", "maxtime"=5,"maxeval"=800000))

#=============================
# Section 2: A harder problem
#=============================
alpha  <- .3
gamma  <- .5
M      <- 30
Time      <- 5
p.x    <- runif(Time,min=2,max=5)
...
UtilHard <- function(x, alpha = .3, gamma = .5, util.type = "cobb", p.x = p.x, p.y = p.y, M=M, beta = .95, Time= Time) {
  ...
  if (util.type == "cobb") {
    util = 0
    for (t in 1:Time) {
      util = util + beta^(t -1 ) * ( (x[t,1]^alpha) * (x[t,2]^gamma ))
    }
    return(- log(util))
  }
}
...
BudgetConstHard <- function(x, M, p.x = p.x, beta = .95, Time=Time, p.y = p.y, alpha = .3, gamma = .5, util.type = "cobb") {
  ...
  #
  #Args: x is a list of x and y quantities. p.x is price of x[1] p.y is price of x[2]
  #      M is income. beta is the discount / borrowing rate Ther other args defineed in the outcome
  #
  # Returns: the negative budget surplus
  # Error Handling:
  x <- matrix(x,Time,2)
...
sol.hard <- solnp(pars = rep(1,2*Time), fun=UtilHard, ineqfun=BudgetConstHard, ineqLB=0, ineqUB=Inf,
                    LB=rep(0,Time*2), p.x = p.x, p.y = p.y, alpha = alpha, M= M, 
                    beta = .95, Time = Time, util.type="cobb")

# Plotting Results
x.hat <- matrix(sol.hard$pars,Time,2)
plot(c(1:Time), x.hat[,1], type="l", col ="black", lwd=5, ylim = c(0,5), ylab="", xlab="")
par(new=T)
plot(c(1:Time), x.hat[,2], type="l", col = "blue", lwd = 5, ylim = c(0,5), ylab="Goods", xlab="Time")